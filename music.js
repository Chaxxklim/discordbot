...
module.exports = {
    ...
    async execute(message, args){
        ...
        if(args[0] == "재생"){
            ...
            if(!voiceChannel){
                return message.reply("명령어를 사용하기 위해선 음성채널에 들어가 있어야합니다.")
            }
            if(status != null){
                if(voiceChannel.id != client.voice?.channel.id){
                    if(client_voice_status == 1 || MPL[0] != null){
                        return message.reply("다른 채널에서 노래 재생중이거나 리스트에 재생곡이 남아있습니당");
                    }
                }
            }
            if(!args[1]){
                ...
                } else if(client_voice_status == 0){
                    return music_play(message);
                } else{
                    return message.reply("이미 노래가 재생되고 있습니다.")
                }
            } else{
                message.reply("유튜브에서 찾을 노래 제목 혹은 URL을 입력하세요.")
            }
            const music = await search_youtube_music(message.content);
            if(!music){
                return message.reply("검색 결과가 없습니당");
            }
            const data = {title : music.title, url : music.url}
            PlaylistArray.push(data);
            if(status != null){
                message.reply(new Discord.message()
                .setTitle("플레이리스트에 노래를 추가했습니다.")
                .setDescription("!노래 플레이리스트 확인 명령어로 현재 플레이리스트를 확인할 수 있습니다.")
                .setColor("#33ff73"));
                if(client_voice_status == 0){
                    return music_play(message);
                }
            } else{
                const permission = voiceChannel.permissionsFor(message.client.user);
                if(!permission.has('CONNECT')){
                    return message.reply("음성 채널 연결 권한이 없습니다.");
                } else if(permission.has('SPEAK')){
                    return message.reply("본 명령어에서는 음성 채널에 말하기 권한이 필요합니다.");
                }
                await voiceChannel.join();
                music_play(message);
            }
        } else if(args[0] == "종료"){
            if(MPL[0] != null || client_voice_status == 1){
                return message.reply("모든 플레이리스트가 끝나지 않았습니다.");
            } else if(status == null){
                return message.reply("이미 노래가 종료되어 있습니다.");
            } else if(!IsJoinVoiceChannel(message)){
                return;
            }
            PlaylistArray = [];
            ...
            switch(args[1]){
                case "삭제" :
                    PlaylistArray = Playlist.get(MGI).get("musicplaylist");
                    if(!args[2] || isNaN(args[2] || args[2] < 0 )){
                        return message.reply("플레이리스트에서 삭제할 올바른 노래 번호를 입력하세요.");
                    }
                    Playlist.get(MGI).set("musicplaylist", PlaylistArray.splice(--args[2], --args[2]));
                    message.reply(new Discord.MessageEmbed()
                    .setTitle("플레이리스트의 항목을 삭제했습니다")
                    .setDescription("!노래 플레이리스트 확인"));
                    break;
                case "확인" :
                    const Embed = new Discord.MessageEmbed()
                    .setTitle("현재 플레이리스트 입니다")
                    .setColor("#009dff");
                    if(Playlist.get(MGI).get("musicplaylist") == null 
                    || Playlist.get(MGI).get("musicplaylist")[0] == null){
                        return message.reply("표시할 플레이리스트가 없습니다.");
                    }
                    PlaylistArray = Playlist.get(MGI).get("musicplaylist");
...
async function music_play(message) {
    ...
    if(!Playlist.get(MGI).get("musicplaylist")[0]){
        message.reply(new Discord.MessageEmbed()
        .setTitle("플레이리스트의 끝입니다")
        .setDescription("노래를 종료합니다."))
    ...
    PlaylistArray.shift();
    Playlist.get(MGI).set("musicplaylist", PlaylistArray);
    message.reply(new Discord.Embed().setTitle("유튜브에서 노래를 재생했습니다.")
                                    .setDescription(`현재 재생곡 : [${music.title}](${music.url})`)
                                    .setColor("#33ff7e"));
...
function IsJoinVoiceChannel(message){
    if(!message.member.voice.channel){
        message.reply("본 명령어를 사용하기 위해선 음성 채널에 접속해야 합니다.");
        return false;
    }
